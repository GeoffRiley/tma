...
\fancyhead[L]{\textsf{ou-tma.sty} notes}
\fancyhead[C]{}
...
\begin{document}
	...
	\hfill\fbox{Update for 2025-04-07 v1.20}
	...
	The contents of the file in which you write a TMA should look something like this:
	\begin{verbatim}
		\documentclass[a4paper,12pt]{article}
		\usepackage{ou-tma}
		%% or include options [roman] or [alph]:
		% \usepackage[roman]{ou-tma} % For Roman numerals in subparts
		% \usepackage[alph]{ou-tma}  % For the default alphabetical letters in subparts
	...
	When requesting the \textsf{ou-tma}\ package with the \verb|\usepackage| command, it is possible to pass one or 
	...
	\verb|\documentclass| have options specifying the paper and font size, so can many other packages being given options.
        ...
        \begin{table}[h]
			...
			\hline
		\end{tabular}
		\caption{Options available for \texttt{ou-tma.sty}}
		\label{tab:options}
	\end{table}
	...
	\subsection*{Backward compatibility}
        ...
        \begin{table}[t]
			...
			\verb|\Fix|        & $\Fix $     &  \\ \hline
		\end{tabular}
		\caption{Mathematical commands that are made available with \texttt{ou-tma.sty}}
		\label{tab:commands}
	\end{table}
	...
	Some standard packages are automatically loaded when the \textsf{ou-tma} package is used. These, in turn, load other packages. A summary of the packages so loaded is listed in table~\ref{tab:packages}.
