...
\begin{document}
	...
	\begin{description}
		\item[\texttt{alph}]\opt[1]{alph} (default) Question numbering as 1(b)(iii).
		\item[\texttt{roman}]\opt[1]{roman} Question numbering as 1(ii)(c).
		\item[\texttt{cleveref}]\opt[1]{cleveref} Enables automatic referencing with the \texttt{cleveref} package.
		\item[\texttt{pdfbookmark}]\opt[1]{pdfbookmark} Adds PDF bookmarks for each question using the \texttt{hyperref} package.
		\item[\texttt{legacy}]\opt[1]{legacy} Enables old definitions of \cmd{vec} and \cmd{C} for backward compatibility with earlier versions of the package. \emph{Deprecated}.
	\end{description}
	...
	Set your personal information\index{personal information}\cmd[1]{myname}\cmd[1]{mycourse}\cmd[1]{mytma}\cmd[1]{mypin}:
	...
	If you wish to set a specific date\cmd[1]{setdate} for your report use:

	\begin{verbatim}
		\setdate{21 November 2024}
	\end{verbatim}

	This is a free format field, any rendition of `date' may be given. You can retrieve the report date later using the \cmd{thedate} command.

	\section{Commands and Environments}

	\subsection{Question Environment}

	Begin answering a question with the \env{question} environment:
